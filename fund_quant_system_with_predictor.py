...
# 导入预测器模块
try:
    from fund_realtime_predictor import FundRealTimePredictor
    PREDICTOR_AVAILABLE = True
except ImportError:
    PREDICTOR_AVAILABLE = False
    logger = logging.getLogger(__name__)
    logger.warning("fund_realtime_predictor模块未找到，实时预测功能将不可用")
...
class FundQuantSystem:
    def __init__(self, tushare_token: str = None):
        ...
        if PREDICTOR_AVAILABLE:
            self.predictor = FundRealTimePredictor(self.tushare_token)
        else:
            self.predictor = None
    ...
    def get_realtime_prediction(self, fund_code: str) -> Optional[Dict]:
        """获取实时估值预测"""
        if self.predictor is None:
            return None
        
        try:
            prediction = self.predictor.predict_fund_value_change(fund_code)
            return prediction
        except Exception as e:
            logger.error(f"获取基金{fund_code}实时预测失败: {e}")
            return None
    ...
    def analyze_fund(self, fund_code: str) -> Dict:
        ...
        try:
            ...
            if self.predictor is not None:
                realtime_prediction = self.get_realtime_prediction(fund_code)
...
def main():
    ...
    print("=== 基金量化分析系统（增强版 - 支持QDII基金 + 实时估值预测）===")
    ...
    if system.predictor is not None:
        ...
        print("实时预测功能: ❌ 不可用（缺少fund_realtime_predictor模块）")
    print()
    ...
    for fund in test_funds:
        ...
        if result["status"] == "success":
            ...
            # 显示实时预测结果
            if result.get('realtime_prediction'):
                pred = result['realtime_prediction']
                if pred['success']:
                    print(f"实时预测: {pred['predicted_change_pct']:+.2f}% (置信度: {pred['confidence']:.2%})")
                    print(f"持仓覆盖率: {pred['holdings_coverage']:.2%}")
                else:
                    print(f"实时预测: 失败 - {pred.get('error', '未知错误')}")
        else:
