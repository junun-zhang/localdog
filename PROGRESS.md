...
- [x] 重复事件例外处理 (编辑单次出现的重复事件 - 拆分法)
- [x] E2E测试全部通过 (导航/滑动/创建事件 共24步)
- [ ] MonthView事件数据接入 (显示实际事件标记)
...
2. 测试手机通知栏遮挡 - 需要预启动后运行Maestro测试
3. Maestro不支持中文inputText，测试中使用默认标题"新事件"

## 下一步
1. 连接MonthView事件数据到EventRepository
2. 实现事件详情页点击编辑导航
3. 实现DayView事件点击导航到详情页
