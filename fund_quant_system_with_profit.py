...
try:
    ...
    REALTIME_PROFIT_AVAILABLE = True
except ImportError:
    REALTIME_PROFIT_AVAILABLE = False
    logger = logging.getLogger(__name__)
    logger.warning("fund_realtime_profit模块未找到，实时收益预估功能将不可用")
...
class FundQuantSystem:
    def __init__(self, tushare_token: str = None):
        ...
        # 初始化实时收益预估器
        if REALTIME_PROFIT_AVAILABLE:
            self.profit_calculator = FundRealTimeProfit()
        else:
    ...
    def calculate_realtime_profit(self, fund_code: str, holding_amount: float) -> Dict:
        """
        计算单个基金的实时收益（新功能）
        ...
        return self.profit_calculator.calculate_profit(fund_code, holding_amount)
    
    def calculate_realtime_profits(self, holdings: List[Dict[str, float]]) -> List[Dict]:
        """
        批量计算多个基金的实时收益（新功能）
        ...
        """
        if not self.profit_calculator:
            return [{'success': False, 'error': '实时收益预估功能不可用'} for _ in holdings]
        
        return self.profit_calculator.calculate_profits(holdings)
...
def main():
    ...
    print("=== 基金量化分析系统（增强版 - 支持QDII基金 + 实时收益预估）===")
    ...
    if REALTIME_PROFIT_AVAILABLE:
        print("=== 实时收益预估测试 ===")
        profit_result = system.calculate_realtime_profit("000001", 10000)
        if profit_result['success']:
            print(f"基金: {profit_result['fund_name']} ({profit_result['fund_code']})")
            print(f"持有金额: ¥{profit_result['holding_amount']:,.2f}")
            ...
            print(f"实时收益预估失败: {profit_result['error']}")
        print("-" * 50)
        print()
